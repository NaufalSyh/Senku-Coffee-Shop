...
// Toggle Populer/Non-Populer Produk
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['toggle_populer_produk_id'])) {
    $id = $_POST['toggle_populer_produk_id'];
    $current = $_POST['current_populer']; // 0 atau 1 dari form
    $new_populer = $current ? 0 : 1;

    $stmt = $conn->prepare("UPDATE produk SET is_populer = ? WHERE id_produk = ?");
    $stmt->bind_param("ii", $new_populer, $id);
    $stmt->execute();
    $stmt->close();
...
// Fungsi untuk menetapkan produk sebagai populer
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['set_populer_id'])) {
    $id_produk = $_POST['set_populer_id'];

    $stmt = $conn->prepare("UPDATE produk SET is_populer = 1 WHERE id_produk = ?");
...
// Fungsi untuk menghapus status populer dari produk
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['unset_populer_id'])) {
    $id_produk = $_POST['unset_populer_id'];

    $stmt = $conn->prepare("UPDATE produk SET is_populer = 0 WHERE id_produk = ?");
    $stmt->bind_param("i", $id_produk);
    $stmt->execute();
    $stmt->close();
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 mb-3">
                                <div class="card h-100">
                                    ...
                                    <div class="card-footer d-flex justify-content-between">
                                        <a href="kelola_promosi.php?id=<?= $row['id_promosi'] ?>" class="btn btn-warning btn-sm"><i class="bi bi-pencil"></i> Edit</a>
                                        <!-- <form method="post" style="display:inline;">
